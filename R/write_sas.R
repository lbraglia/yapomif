### This file is derived from the 'foreign' package for R.
...
make.SAS.formats <- function(varnames){
    x <- gsub("_", "", varnames)     # this is changed
    x <- sub("^([0-9])", "_\\1", x) 
    x <- gsub("[^a-zA-Z0-9_]", "_", x)
    x <- sub("([0-9])$", "\\1f", x) # can't end in digit so append 'f'

    ## below changed as well
    xal <- abbreviate(x, minlength = 8L, method = "left.kept")
    xal_wrong <- any(nchar(xal) > 8L) || any(duplicated(xal))

    if (! xal_wrong) {
        names(xal) <- varnames
        return(xal)
    } else {
        xab <- abbreviate(x, minlength = 8L, method = "both.sides")
        xab_wrong <- any(nchar(xab) > 8L) || any(duplicated(xab))
        if (! xab_wrong) {
            names(xab) <- varnames
            return(xab)
        } else {
...
#' Slightly enhanced writeForeignSAS
#'
#' This version manages factor labels (make.SAS.formats) differently
#' @param df same as writeForeignSAS
#' @param datafile same as writeForeignSAS
#' @param codefile same as writeForeignSAS
#' @param dataname same as writeForeignSAS
#' @param validvarname same as writeForeignSAS
#' @param libpath same as writeForeignSAS 
#' @export
