...
test_that("%inRange% does what it suggests", {

  ## integer
  expect_that(1L %inRange% c(2L,4L), is_false())
  expect_that(3L %inRange% c(2L,4L), is_true())
  expect_that(NA %inRange% c(2L,4L), is_identical_to(NA))

  ## double
  expect_that(1.0 %inRange% c(2.0, 4.0), is_false())
  expect_that(3.0 %inRange% c(2.0, 4.0), is_true())
  expect_that(NA %inRange%  c(2.0, 4.0), is_identical_to(NA))
